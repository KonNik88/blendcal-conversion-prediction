...
# ---------- Helpers: совместимость разных версий Streamlit ----------
...
def _img(path: Union[str, Path]):
    """Показать изображение с поддержкой старых/новых параметров ширины."""
    try:
...
def _df(df: pd.DataFrame, **kwargs):
    """Показать dataframe c фоллбэком по параметрам."""
    try:
        return st.dataframe(df, use_container_width=True, **kwargs)
    except TypeError:
        # выкидываем неподдерживаемые ключи
...
def _download_btn(label: str, *, data: bytes, file_name: str, mime: str):
    """Кнопка скачивания с фоллбэком по ширине."""
    try:
...
def _button(label: str, **kwargs) -> bool:
    """Кнопка: сначала пробуем use_container_width, иначе – без него."""
    try:
        return st.button(label, use_container_width=True, **kwargs)
    except TypeError:
        return st.button(label, **kwargs)

def _form_submit(label: str, **kwargs) -> bool:
    """Кнопка внутри формы: фоллбэк по ширине."""
    try:
    ...
    with colA:
        if _button("Проверить API"):
            try:
                ...
                st.error(e)
    with colB:
        refresh = _button("Обновить версию")
...
with tab_single:
    ...
    with col_p3:
        if _button("⚡ Auto-warm from /cat_top"):
            changed = 0
            ...
            _safe_rerun()
    with col_p4:
        auto_trig = st.checkbox("Автосчитать hour_sин/hour_cos и dow_sин/dow_cos из hour/dow", value=True)

    # --- JSON «подмешать» ---
    with st.expander("Дополнительно: подмешать JSON"):
        allow_json_override = st.checkbox("Разрешить JSON переопределять режим порога (mode)", value=False)
    ...
    with st.form("single_form"):
        ...
        if isinstance(extra_dict, dict):
            ej = extra_dict.get("features", extra_dict)
        ...
        with cL:
            refresh_diag = _form_submit("Обновить диагностику")
        with cM:
            submitted = _form_submit("Предсказать", type="primary")
        with cR:
            clear = _form_submit("Очистить")
            if clear:
                st.session_state.form_values = {}
                _safe_rerun()
        with cD:
            dbg_req = _form_submit("🔎 Debug prediction")
...
with tab_batch:
    ...
    if f is not None and _button("Скорить CSV"):
        try:
            ...
            if resp.status_code != 200:
                ...
                _download_btn("⬇️ Скачать результат", data=resp.content, file_name=out_name, mime="text/csv")
                try:
                    df_preview = pd.read_csv(StringIO(resp.content.decode("utf-8"))).head(50)
                    _df(df_preview, hide_index=True)
                except Exception:
                    pass
        except Exception as e:
            st.exception(e)
